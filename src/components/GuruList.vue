    ...
    <ul class="play-body">
      <li class="card-frame" v-for="(item, index) in playList" :key="index">
        <div class="card" @click="goAblum(item)">
          <el-image class="card-img" fit="contain" :src="getImageUrl(item.pic)" />
          <div class="mask" @click="goAblum(item)">
            <baoyan-icon class="mask-icon" :icon="LIANHUA"></baoyan-icon>
          </div>
        </div>
...
.mask {
  ...
  .mask-icon {
    @include icon(1.5em, rgba(240, 240, 240, 1));
  }
